...
def do_training(data_dir, model_dir, device, image_size, input_size, num_workers, batch_size,
                learning_rate, max_epoch, save_interval, val_interval, wandb_name):
    ...
    for epoch in range(max_epoch):
        ...
        if (epoch + 1) % val_interval == 0 or epoch >= max_epoch - 5:
            with torch.no_grad():
                ...
                receipt_dirs = [osp.join(data_dir,d) for d in os.listdir(data_dir) if d.endswith('_receipt')]
        ...
        if (epoch + 1) % save_interval == 0:
            ...
            ckpt_fpath = osp.join(model_dir, f'{wandb_name}_latest.pth')
